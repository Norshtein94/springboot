package com.dada.shen.springboot.zookeeper.distributedlock;
    ...
    public static void main(String[] args) {
        Runnable task1 = new Runnable(){
            public void run() {
                ...
                try {
                    lock = new DistributedLock("120.55.161.125:2182","test1");
        ...
        for(int i=0;i<tasks.length;i++){
            ConcurrentTest.ConcurrentTask task3 = new ConcurrentTest.ConcurrentTask(){
                public void run() {
                    DistributedLock lock = null;
                    try {
                        lock = new DistributedLock("120.55.161.125:2183","test2");
                        lock.lock();
                        System.out.println("Thread " + Thread.currentThread().getId() + " running");
                    } catch (Exception e) {
                        e.printStackTrace();
